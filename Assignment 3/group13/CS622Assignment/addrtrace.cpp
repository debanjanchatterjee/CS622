...
VOID PrintTrace(THREADID tid,UINT64 addr, UINT32 size, CHAR T){
...
if(addr+memaccesssize<=presentmemblockend || (addr==presentmemblockstart && memaccesssize<=64)){
	UINT blocksize=0;
	while(1){
	fprintf(trace,"%d %lu %c %lu\n",tid,addr+blocksize,T,globalcount);
	globalcount++;
	tracecount++;
	fflush(trace);
	blocksize+=8;
